...
function processNode(node, parentKey) {
  ...
  if (isBlockElement(nodeName)) {
    ...
    node.childNodes.forEach((childNode, index) => {
      ...
      if (isInlineElement(childNode.nodeName) || isText(childNode.nodeName)) {
        ...
        if (lastInlineNodes.length > 0) {
          children.push(<Text key={`${key}_${index}_inline`} style={styleForTag(nodeName)}>{lastInlineNodes}</Text>)
          lastInlineNodes = []
        }
        children.push(child)
      }
    ...
    if (lastInlineNodes.length > 0) {
      children.push((<Text key={`${key}_last_inline`} style={styleForTag(nodeName)}>{lastInlineNodes}</Text>))
    }
    return (
      <View key={key}>
      {children}
